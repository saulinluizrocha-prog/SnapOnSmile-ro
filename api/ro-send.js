/**
 * Vercel Serverless Function — /api/ro-send
 * GEO: Romania (RO)
...
export default async function handler(req, res) {
  ...
  // Reject clearly invalid phones — but be lenient to avoid silent loss
  ...
  /* ── CONFIG ── */
  const TOKEN       = process.env.DRCASH_TOKEN || 'YZA0ZJDLZWYTZDK4ZC00YMJJLWJJNJATODZKNGJJMTE2MZQ4';
  const STREAM_CODE = process.env.DRCASH_STREAM_RO || 'u2z8e';
  ...
  try {
    ...
    console.error('[ro-send] fetch error', err);
